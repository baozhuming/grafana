...
const LabelsWithSuggestions: FC<{ dataSourceName: string }> = ({ dataSourceName }) => {
  ...
  return (
    <>
      {loading && <LoadingPlaceholder text="加载" />}
      {!loading && (
        <>
                  ...
                  >
                    <AlertLabelDropdown
                      {...register(`labels.${index}.key`, {
                        required: { value: Boolean(labels[index]?.value), message: '必输字段' },
                      })}
                  ...
                  >
                    <AlertLabelDropdown
                      {...register(`labels.${index}.value`, {
                        required: { value: Boolean(labels[index]?.key), message: '必输字段' },
                      })}
          ...
          <div key={field.id}>
            <div className={cx(styles.flexRow, styles.centerAlignRow)} data-testid="alertlabel-input-wrapper">
              ...
              >
                <Input
                  {...register(`labels.${index}.key`, {
                    required: { value: !!labels[index]?.value, message: '必输字段' },
                  })}
                  placeholder="key"
                  data-testid={`label-key-${index}`}
              ...
              >
                <Input
                  {...register(`labels.${index}.value`, {
                    required: { value: !!labels[index]?.key, message: '必输字段' },
                  })}
                  placeholder="value"
                  data-testid={`label-value-${index}`}
  ...
  return (
    <div className={cx(className, styles.wrapper)}>
      <Label>
        <Stack gap={0.5}>
          <span>自定义标签</span>
          <Tooltip content={<div>下拉列表只显示以前用于警报的标签，从下拉菜单中选择一个标签，或者键入一个新标签</div>}>
